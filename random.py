...
# --------------------------
# MTMini – pseudonáhodný generátor
# --------------------------
class MTMini:
...
# Streamlit UI
# --------------------------
st.title("MTMini – prelomenie, predikcia a šanca aproximácie")

st.markdown("""
- Generovanie pseudonáhodných čísiel a vizualizácie.
- Prelomenie generátora – presné aj aproximované.
- Predikcia ďalšieho čísla.
- Vizualizácia rozptylu a 3D scatter.
- Statistická šanca úspechu aproximácie.
""")
...
if st.button("Predict next number"):
    if pred_type == "Presná" and not st.session_state.broken_exact:
        st.warning("Najprv prelomenie presné.")
    elif pred_type == "Aproximovaná" and not st.session_state.broken_approx:
        st.warning("Najprv prelomenie aproximované.")
    else:
        if pred_type == "Presná":
        ...
        # Evidencia úspechu aproximácie
        if pred_type == "Aproximovaná":
            st.session_state.approx_hits.append(predicted_val == next_val_actual)
...
# --------------------------
# Statistická šanca aproximácie
# --------------------------
st.subheader("Šanca úspechu aproximovanej predikcie")
if st.button("Ukázať šancu"):
    n_bits_state = mt.n * mt.w
    prob_per_bit = 0.5
    prob_next_number_exact = prob_per_bit ** mt.w
    num_outputs = np.arange(1, max(220, len(outputs))+1)
    prob_hit = prob_next_number_exact * np.ones_like(num_outputs)

    fig2, ax = plt.subplots()
    ax.plot(num_outputs, prob_hit, marker='o')
    ax.set_xlabel("Počet vygenerovaných čísel")
    ax.set_ylabel("Pravdepodobnosť presného trafenia")
    ax.set_title("Statistická šanca aproximácie")
    ax.grid(True)
    st.pyplot(fig2)

    # Empirická simulácia aproximácie
    if st.session_state.approx_hits:
        hits = sum(st.session_state.approx_hits)
        total = len(st.session_state.approx_hits)
        st.markdown(f"- Počet simulovaných predikcií: {total}")
        st.markdown(f"- Počet úspešných trafení: {hits}")
        st.markdown(f"- Empirická pravdepodobnosť: {hits/total*100:.5f} %")
    st.markdown("""
    - Každý bit vnútorného stavu má 50 % šancu byť správny v aproximácii.
    - Pravdepodobnosť trafiť presne ďalšie číslo je veľmi malá (~0,39 % pre 8-bitový výstup).
    - Preto aj po stovkách generovaných čísiel aproximácia často netrafí.
    - Presná predikcia vyžaduje známy vnútorný stav (presné prelomenie).
    """)
